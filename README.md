...
###### [git hub - flow?](https://hyeonic.tistory.com/220)
###### [git-flow?](https://techblog.woowahan.com/2553/)
###### [git flow vs git hub flow](https://hellowoori.tistory.com/56)
...
|내용|사람|기간|
|------|---|---|
|아이디어 회의|재빈, 상윤, 희수, 우형|4/1 ~ 4/6|
|전체 프로젝트 기획 및 구상|재빈, 상윤, 희수, 우형|4/4 ~ 4/15|
|프로젝트 총괄, setup |재빈|4/10 ~ 4/13|
|DB 설계|재빈, 상윤|4/5 ~ 4/7|
|데이터 추가|상윤|4/7 ~4/21|
|DB 연동 테스트|재빈|4/10 ~ 4/13|
|스토리 보드 , 플로우 차트|희수, 우형|4/6 ~ 4/10|
...
|내용|사람|기간|
|------|---|---|
|레시피 게시물 목록 전체조회, 무한스크롤(커서 페이지 네이션), 레시피 최신순(날짜순) 조회|재빈|4/13~15|
|레시피 게시물 저장, 재료 검색, 다중이미지 파일업로드|재빈|4/15~4/18|
|레시피 게시물 상세조회, 레시피 게시물 조회수순 조회|상윤|4/16~4/22|
|회원가입, 이메일/닉네임 중복검사|희수|4/13~4/18|
|jwt + 쿠키를 이용한 로그인/로그아웃|우형|4/13~4/18|
|냉장고 재료를 이용한 레시피 검색|재빈|4/18~4/22|
|제목을 이용한 레시피 검색|재빈|4/18~4/22|
|회원의 레시피 장바구니 조회, 저장, 삭제|우형,상윤|4/22~4/24|
|프로필 조회, 프로필 수정|희수|4/18~4/22|
|레시피 투표 저장|우형|4/22~4/24|
|전 일주일 투표 집계후 인기게시물 저장(mysql 스케줄러 + 프로시저)|희수|4/22 ~ 4/24|
|인기게시물 best 10 조회|희수|4/22 ~ 4/24|
|에러검출, 각자의 기능 블랙박스테스트|재빈->상윤, 우형->재빈, 희수->우형, 상윤->희수|4/24|
...
- 이메일, 닉네임 중복검사
- 로그인 -- jwt token 인증, 인가 / 브라우저 쿠키를 이용
- 로그아웃
- 프로필 조회 -> mypage
- 프로필 수정 -> mypage

- 레시피 게시물 목록 전체조회
- 레시피 게시물 무한 스크롤 -- 커서 페이지 네이션
- 레시피 게시물 최신순(날짜순) 조회 

- 레시피 게시물 조회수순 조회 
- 레시피 게시물 추천순(좋아요수순) 조회  --- 좋아요 개수 부족으로 x 
- 레시피 게시물 상세조회

- 레시피 게시물 저장
  - 회원ID, 제목, 내용, 난이도, 조리시간ID, 순위, 부재료, 태그명, 종류별ID
- 레시피 게시물 다중 이미지 파일업로드 
 
- 레시피 투표 저장-> 가장 인기있는 BEST10으로 이달의 레시피, 이주의 레시피 
- 레시피 투표 집계후 인기게시물 BEST10 저장 - mysql 스케줄러 사용
- 레시피 랭크 게시물 조회 -> 인기 게시물 BEST10 조회: 이달의 레시피, 이주의 레시피 조회

- 레시피 냉장고 재료별 검색 : 내가 가지고 있는 재료를 이용하여 내가 가지고 있는 재료별 검색 ★ 핵심기능
- 레시피 게시물 제목별 검색 :  

- 레시피 게시물 장바구니 저장, 조회, 삭제

### 2순위 --- 개발 기간이 5일 줄어듬에 있어서 2순위 부터 진행하지 못했습니다.
- 레시피 게시물 종류별 검색
