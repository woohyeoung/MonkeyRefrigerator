...
function BoardCreate() {
  ...
  // 제출 직전 썸네일 선택
  const onSubmitThumbnail = async (index) => {
    let a = window.confirm("최종 등록 하시겠습니까?");
    if (a) {
      let file = pictures[0][index];
      let newPictures = [...pictures[0]];

      newPictures.splice(index, 1);
      newPictures.splice(0, 0, file);

      let arr = Array.from(Array(1), () => new Array(pictures[0].length));
      for (let i = 0; i < arr[0].length; i++) {
        arr[0][i] = newPictures[i];
      }
      setPictures([arr]);
      onSubmit();
    } else {
        ...
        <Box sx={Modalstyle2}>
          ...
          >
            ...
            >
              {dataURLs ? (
                <>
                  {dataURLs[0]?.map((item, index) => {
                    return (
                      <Card
                        key={index}
                        onClick={() => {
                          onSubmitThumbnail(index);
                        }}
                        width={"300px"}
                        height={"300px"}
          ...
          {/* <Button onClick={} onSubmitThumbnail>
            썸네일 최종 선택 완료
          </Button> */}
        </Box>
      </Modal2>
